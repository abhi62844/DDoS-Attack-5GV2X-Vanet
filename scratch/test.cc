    ...
    */
    ...
    AnimationInterface anim ("5g-nr-animation.xml");
    for (uint32_t i = 0; i < enbNodes.GetN (); ++i)
    {
        anim.UpdateNodeColor (enbNodes.Get (i), 255, 0, 0); // (Red, Green, Blue)
    }

    // Change color for UE nodes (e.g., Blue)
    for (uint32_t i = 0; i < ueNodes.GetN (); ++i)
    {
        anim.UpdateNodeColor (ueNodes.Get (i), 0, 0, 255); // (Red, Green, Blue)
    }
    ...
    anim.UpdateNodeDescription (enbNodes.Get (0), "gNB-1");
    anim.UpdateNodeDescription (enbNodes.Get (1), "gNB-2");
    anim.UpdateNodeDescription (ueNodes.Get (0), "UE-1");
    anim.UpdateNodeDescription (ueNodes.Get (1), "UE-2");
    anim.UpdateNodeDescription (pgw, "PGW");
    anim.SetConstantPosition (enbNodes.Get (0), 0, 0);
    anim.SetConstantPosition (enbNodes.Get (1), 0, 80);
    anim.SetConstantPosition (ueNodes.Get (0), 90, 15);
    anim.SetConstantPosition (ueNodes.Get (1), 30, 50);
    anim.EnablePacketMetadata (true);    // Capture packet transmission details
    anim.EnableIpv4RouteTracking ("routing-table.xml", Seconds (0), Seconds (simTime), Seconds (0.25));  // Track IPv4 routes 
